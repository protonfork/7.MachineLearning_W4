...
sub.training <- scale(sub.training,center=TRUE,scale=TRUE);
```
...
cor.sub.f.training <- cor(sub.filtered.training)
corrplot(cor.sub.f.training, order = "hclust", tl.col="black", tl.cex=.45)
```
...
```{r training models : random forest (ranger package) }
...
ranger.fit <- train(y=train.labels,x=train.set, method = "ranger", trControl = ctrl, metric = "Accuracy", preProc = c("center", "scale"), importance = "permutation", verbose=F)
```

```{r training models : XG Boost}
...
XgBoost selected model has sort the feature by importance for the classification of the observations. Here under table gives some mode details on the feature that are more impacting for the classification.

```{r Importance of Features}
xgb.importance(names(train.set),model=xgb.fit)
```
...
t_validation <- validation %>% select_if(is.numeric) %>% 
        select_if(function(col)all(!is.na(col))) %>%
        dplyr::select(-raw_timestamp_part_1,-raw_timestamp_part_2,-num_window,-X)

pred.xgb <- predict(xgb.fit,newdata=data.matrix(t_validation[names(t_training)]))
pred.xgb <- matrix(pred.xgb,nrow=length(pred.xgb)/5,ncol=5,byrow=T)
